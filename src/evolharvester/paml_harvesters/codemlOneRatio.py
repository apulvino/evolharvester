...
# OneRatio-specific tree lengths (per-dN and per-dS)
...
TREE_LENGTH_RE   = re.compile(r'tree length\s*=\s*([0-9\.]+)')  # keep older fallback
...
def parse_codeml_oneratio(path):
    seq_map = {}
    branches = []
    summary = {
        "ns": None,
        ...
        "omega_global": None,
        "tree_length": None,
        "tree_length_dN": None,   # OneRatio-specific
        "tree_length_dS": None,   # OneRatio-specific
        "model": None,
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            # OneRatio-specific: dN/dS tree lengths
            if m := TREE_LENGTH_DN_RE.search(line):
                try:
                    summary["tree_length_dN"] = float(m.group(1))
                except Exception:
                    pass
                continue
            if m := TREE_LENGTH_DS_RE.search(line):
                try:
                    summary["tree_length_dS"] = float(m.group(1))
                except Exception:
                    pass
                continue

            # fallback legacy tree length capture if present
    ...
    # If OneRatio provided separate dN/dS tree lengths, optionally set tree_length to their sum
    if summary.get("tree_length_dN") is not None and summary.get("tree_length_dS") is not None:
        try:
            summary["tree_length"] = summary["tree_length_dN"] + summary["tree_length_dS"]
        except Exception:
            pass
...
def write_csv(branches_all, outpath):
    # NOTE: only added two codon-derived columns + OneRatio-specific tree_length_dN/tree_length_dS; all other fields left unchanged
    fields = [
...
def run(input_path: str, output_path: str, *, verbose: bool = False, **kwargs) -> int:
    """
    Run codeml OneRatio harvester. Wraps parse_codeml_oneratio() with file
    ...
    if in_path.is_file():
        ...
        gene_layout = sorted(in_path.glob("*/codeml/OneRatio.out"))
        flat_layout = sorted(in_path.glob("OneRatio.out"))
        files = gene_layout if gene_layout else flat_layout
        if not files:
            files = sorted(in_path.rglob("OneRatio.out"))
    else:
    ...
    if not files:
        if verbose:
            print(f"[codeml-oneratio] no OneRatio.out files found at {input_path}")
        return 0

    # Determine output file
    if out_path.is_dir():
        out_file = out_path / "codeml_OneRatio_filtered_stats.csv"
    else:
        out_file = out_path
    ...
    for f_path in files:
        ...
        try:
            seq_map, branches, summary = parse_codeml_oneratio(f_path)
            for b in branches:
                row = {**b, **summary, "seq": gene_name}
                branches_all.append(row)
            if verbose:
                print(f"[codeml-oneratio] parsed {len(branches)} branches for {gene_name}")
        except Exception as e:
            if verbose:
                print(f"[codeml-oneratio] ERROR {gene_name}: {e}")

    write_csv(branches_all, out_file)

    if verbose:
        print(f"[codeml-oneratio] wrote {len(branches_all)} rows to {out_file}")
...
def main():
    ...
    for seq_dir in BASE_DIR.iterdir():
        codeml_path = seq_dir / "codeml" / "OneRatio.out"
        if codeml_path.exists():
            seq_map, branches, summary = parse_codeml_oneratio(codeml_path)
            for b in branches:
                b_row = {**b, **summary, "seq": seq_dir.name}
                branches_all.append(b_row)
            print(f"✔ Parsed {len(branches)} branches for {seq_dir.name}")
        else:
            print(f"⚠ No OneRatio.out found for {seq_dir.name}, skipping")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="codemlOneRatio_harvester", description="Harvest codeml (OneRatio) model outputs to CSV.")
    parser.add_argument("--input", "-i", required=True, help="Input codemlOneRatio.out file, directory, or glob")
    ...
    if args.verbose:
        print(f"✔ codeml-OneRatio model stats written: {count} rows")
