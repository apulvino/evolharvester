...
BASE_DIR = Path("/projects/b1057/apulvino/Chapter2/v4_orthexplorer/results/gene_centric/paml")
OUTPUT_FILE = "codeml_M1a_branches_all.csv"
...
TREE_LENGTH_RE = re.compile(r'tree length\s*=\s*([0-9\.]+)')
MODEL_RE = re.compile(r'Model:\s*(.*)')
CODON_FREQ_RE = re.compile(r'Codon frequency model:\s*(.*)')

# M1a-specific: site-class proportions and omegas
...
def parse_codeml_m1a(path):
    ...
    summary = {
        ...
        "lnL": None,
        "kappa": None,
        "omega_global": None,   # populated from p/w if possible
        "p_siteclasses": None,
        "w_siteclasses": None,
        "tree_length": None,
        "model": None,
        ...
        # --- codon fields added ---
        "codon_pos_base_seq": [],   # list of per-sequence dicts
        ...
        for line in fh:
            ...
            if m := SEQNUM_RE.match(line):
                try:
                    ...
                    pass
                # also PRIME codon-pos per-sequence capture (compatible with your other scripts)
                try:
                    idx = int(m.group(1))
            ...
            # M1a site-class lines
            if m := P_LINE_RE.match(line):
                try:
                    p_vals = [float(x) for x in m.group(1).split()]
                    # KEEP existing behavior: p_siteclasses as string (comma-separated)
                    summary["p_siteclasses"] = ",".join(map(str, p_vals))
                except Exception:
                    pass
                continue
            ...
            if m := W_LINE_RE.match(line):
                try:
                    ...
                    # KEEP existing behavior: w_siteclasses as string (comma-separated)
                    summary["w_siteclasses"] = ",".join(map(str, w_vals))
                except Exception:
                    pass
                continue
    ...
    # compute global omega ONLY if p and w exist
    if p_vals and w_vals and len(p_vals) == len(w_vals):
        try:
            summary["omega_global"] = sum(p * w for p, w in zip(p_vals, w_vals))
        except Exception:
            summary["omega_global"] = None
...
def write_csv(branches_all, outpath):
    ...
    with open(outpath, "w", newline="", encoding="utf-8") as fh:
        ...
        for row in branches_all:
            # keep other columns' behavior unchanged; codon fields are lists/tuples as populated
            w.writerow(row)

def run(input_path: str, output_path: str, *, verbose: bool = False, **kwargs) -> int:
    """
    Run codeml M1a harvester. Wraps parse_codeml_m1a() with file discovery
    and output handling.
    """
    ...
    if in_path.is_file():
        ...
        gene_layout = sorted(in_path.glob("*/codeml/M1a.out"))
        flat_layout = sorted(in_path.glob("M1a.out"))
        files = gene_layout if gene_layout else flat_layout
        if not files:
            files = sorted(in_path.rglob("M1a.out"))
    else:
    ...
    if not files:
        if verbose:
            print(f"[codeml-m1a] no M1a.out files found at {input_path}")
        return 0

    # Determine output file
    if out_path.is_dir():
        out_file = out_path / "codeml_M1a_filtered_stats.csv"
    else:
        out_file = out_path
    ...
    for f_path in files:
        ...
        try:
            seq_map, branches, summary = parse_codeml_m1a(f_path)
            for b in branches:
                row = {**b, **summary, "seq": gene_name}
                branches_all.append(row)
            if verbose:
                print(f"[codeml-m1a] parsed {len(branches)} branches for {gene_name}")
        except Exception as e:
            if verbose:
                print(f"[codeml-m1a] ERROR {gene_name}: {e}")

    write_csv(branches_all, out_file)

    if verbose:
        print(f"[codeml-m1a] wrote {len(branches_all)} rows to {out_file}")
...
def main():
    ...
    for seq_dir in BASE_DIR.iterdir():
        codeml_path = seq_dir / "codeml" / "M1a.out"
        if codeml_path.exists():
            seq_map, branches, summary = parse_codeml_m1a(codeml_path)
            for b in branches:
                b_row = {**b, **summary, "seq": seq_dir.name}
                branches_all.append(b_row)
            print(f"✔ Parsed {len(branches)} branches for {seq_dir.name}")
        else:
            print(f"⚠ No M1a.out found for {seq_dir.name}, skipping")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="codemlM1a_harvester", description="Harvest codeml (M1a model) outputs to CSV.")
    parser.add_argument("--input", "-i", required=True, help="Input codemlM1a.out file, directory, or glob")
    ...
    if args.verbose:
        print(f"✔codeml-M1a model sstats written: {count} rows")
